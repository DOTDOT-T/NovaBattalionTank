...
void TankController::OnUpdate()
{
    ...
    if (InputAPI::isActionDown(1)) owner->AddVelocity(owner->GetTransform()->GetForward() * moveSpeed);
    if (InputAPI::isActionDown(0)) owner->AddVelocity(owner->GetTransform()->GetForward() * -moveSpeed);
